...
- **1 (Critical).** `web/transcripciones/views.py`: la vista `cancion` ahora, si `cancion.estado == Cancion.PENDIENTE` y tiene `origen`, llama a `_preparar_si_hace_falta` y hace `refresh_from_db()` antes de renderizar, en vez de quedarse sondeando para siempre. `cancion_reintentar` acepta también el estado `PENDIENTE` (`estado__in=[Cancion.ERROR, Cancion.PENDIENTE]`). `web/transcripciones/templates/transcripciones/cancion.html`: la rama de sondeo (`{% else %}`) ofrece "Reintentar" cuando `cancion.estado == "pendiente"` (sin `origen`, así que no se lanzó nada).
- **2 (Important).** `web/static/onda.js`: las manijas ya no quedan atadas al nombre con el que se agarraron. Se guarda en `arrastrando` cuál borde se está moviendo y se lo reasigna al otro si el puntero lo cruza, así la selección sigue al dedo en vez de colapsar en una astilla.
- **3 (Important).** `web/static/estilo.css`: `.pista-onda` pasa de `touch-action: none` a `touch-action: pan-y`, para no bloquear el scroll vertical del teléfono (las manijas conservan `none`).
- **4 (Important).** `web/transcripciones/views.py`: `_frase_a_dict` añade la clave `"reintentar"` con la URL de `reintentar`. `web/static/onda.js`: `pintarLista` agrega un botón "Reintentar" para las frases en error que llama a esa URL por POST y refresca la lista. `web/static/estilo.css`: estilos `.estado-pendiente` y `ol.frases button.reintentar`.
- **5.** `web/static/onda.js`: el bucle `requestAnimationFrame(dibujar)` ya no corre indefinidamente. `dibujar` solo se reprograma a sí mismo mientras el audio suena o se arrastra una manija; el resto de los cambios de estado (selección, lista de frases, resize, tiempo, play/seeked) piden un redibujo puntual con la nueva función `programarDibujo` (con bandera `dibujoPendiente` para no encolar más de uno por frame).
- **6.** `web/transcripciones/views.py`: `crear_frase` rechaza `inicio_s` negativo con 400 antes de las demás comprobaciones.
- **7.** `motor/pipeline.py`: nueva constante `TOLERANCIA_LIMITE_S = 0.5`; `analizar_recorte` compara contra `config.max_fragmento_s + TOLERANCIA_LIMITE_S` para no rechazar un recorte de 180 s por los milisegundos de más que deja Demucs.
- **8.** `web/transcripciones/views.py`: la vista `cancion` pasa `frases` como lista de diccionarios con `rango` ya formateado en m:ss (`formato_tiempo`) y `estado` legible. `web/transcripciones/templates/transcripciones/cancion.html`: la lista usa `frase.rango` y `frase.estado` en vez de `floatformat:0` y `get_estado_display`, así queda igual al formato que pinta el JS.
- **10.** `web/static/onda.js`: `refrescarFrases` tiene una bandera `sondeando` que corta la función si ya hay un sondeo en curso, para no solapar peticiones.
- **11.** `web/static/onda.js`: se borró la constante `pista` (`.pista-onda`), que no se usaba.
- **12.** `web/static/onda.js`: los `fetch` de sondeo (`urls.estado` y `urls.onda`, en `arrancar` y en `refrescarFrases`) llevan `{ cache: "no-store" }`. `web/transcripciones/templates/transcripciones/cancion.html` y `detalle.html`: lo mismo en el `fetch` del `<script>` de sondeo.
- **14.** `CLAUDE.md`, sección "Estado": se borró la frase de que la tarea 21 pasaba de opcional a necesaria en iPhone; ya está atendida (`transcripciones/audio_http.py`), y se dejó la mención de que Safari puede exigir `Range`.
- **16.** `web/static/onda.js`: el manejador que restaura el texto de "Escuchar la selección" ahora está en una función nombrada (`restaurarBotonEscuchar`) enganchada a `pause` y también a `ended`.

Pruebas nuevas o ampliadas: `web/transcripciones/tests_vistas.py` (`test_abrir_una_cancion_pendiente_con_origen_la_prepara`, `test_abrir_una_cancion_pendiente_sin_origen_ofrece_reintentar`, `test_reintentar_una_cancion_pendiente_la_prepara`, la aserción de `reintentar` en `test_el_estado_de_la_cancion_trae_sus_frases_en_orden`, el caso de inicio negativo en `test_una_seleccion_invertida_corta_larga_o_fuera_da_400`) y `web/transcripciones/tests_trabajos.py` (`test_la_preparacion_de_la_cancion_corre_fuera_del_semaforo`). Los puntos 2, 3, 5, 10, 11 y 16 son JS/CSS puro, sin test automatizado; el punto 7 lo sigue cubriendo el test existente `test_rechaza_un_fragmento_mas_largo_que_el_limite`.
